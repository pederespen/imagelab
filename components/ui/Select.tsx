import { SelectHTMLAttributes, forwardRef, ReactNode } from 'react'

interface SelectProps extends SelectHTMLAttributes<HTMLSelectElement> {
  label?: string
  error?: string
  helperText?: string
  children: ReactNode
}

export const Select = forwardRef<HTMLSelectElement, SelectProps>(
  ({ label, error, helperText, className = '', children, ...props }, ref) => {
    const selectClasses = `w-full px-3 py-2.5 pr-9 border rounded-lg focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-1 bg-card text-foreground transition-all cursor-pointer text-sm font-medium appearance-none bg-[url('data:image/svg+xml;base64,')] bg-[length:12px] bg-[right_0.75rem_center] bg-no-repeat [&>option]:bg-card [&>option]:text-foreground [&>option]:py-2 ${
      error ? 'border-red-400' : 'border-input hover:border-border'
    } ${className}`

    return (
      <div className="w-full">
        {label && (
          <label className="block text-xs font-medium text-muted-foreground mb-2 uppercase tracking-wide">
            {label}
          </label>
        )}
        <select ref={ref} className={selectClasses} {...props}>
          {children}
        </select>
        {error && <p className="text-xs text-red-600 mt-1.5">{error}</p>}
        {helperText && !error && (
          <p className="text-xs text-muted-foreground mt-1.5">{helperText}</p>
        )}
      </div>
    )
  }
)

Select.displayName = 'Select'
